...
def querystdnotintraineeplan(code):
    ...
    print("querystdnotintraineeplan {}".format(traineestudentlist))
...
# 带教老师列表
def gettraineeteacher(id, tecname):
    url = domain + "/trainee/gettraineeteacher"

    data = {"1":{"command":"trainee/gettraineeteacher","sessionid":getSessionId(),"loginid":"1","page":1,"pagesize":10,"id":id,"tecname": tecname}}
...
def traineedetaillistallwait(stdinfo):
    ...
    data = {"1":{"command":"trainee/traineedetaillistall","sessionid":getSessionId(),"loginid":"1","uid":"1","levelcode":"position","levelkey":"9","iscontroldept":"1","pagesize":1,"page":1,"tecname":"","grade":"","deptid":"","yearmonth":"","turnstatus":0,"starttime":"2022-01-01","endtime":"2025-12-12","stdinfo":stdinfo}}
...
def add():
    ...
    std_type_dict = {"1": {"name": "留学生", "code": "YCLXS"}, "2": {"name": "本科生", "code": "YCBKS"},
                     "3": {"name": "外院", "code": "YCWY"}}
    std_type = "2"
    std_name = std_type_dict[std_type]["name"]
    std_code = std_type_dict[std_type]["code"]

    for i in range(744, 50001):
        code = std_code + "{}".format(i).zfill(5)
        turndept = random.choice(turndeptlist)
        datetuple = random.choice(datetuplelist)
        ...
        if len(traineedetaillistallwails) > 0:
            ...
            try:
                indept(id)
                # 再分配带教
                tecname = "压测实习生带教" + "{}".format(random.randint(0, 1000)).zfill(5)
                tecid = gettraineeteacher(id, tecname)[0]
