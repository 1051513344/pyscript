...
状态
    ...
    if package is None or package == "":
        package = "package cn.ucmed.yijiao.base.domain.vo;"
    ...
    with open("{}.java".format(className), "w", encoding="utf-8") as f:
        ...
        f.write("import com.alibaba.excel.annotation.ExcelIgnoreUnannotated;\n")
        f.write("import com.alibaba.excel.annotation.ExcelProperty;\n\n")
        f.write("/**\n * @author {}\n * @date {}\n */\n\n".format(author, date))
        f.write("@Data\n")
        f.write("@ExcelIgnoreUnannotated\n")
        ...
        f.write("     * {}\n".format("id"))
        f.write("     */\n")
        f.write("    private {} {};".format("String", "id"))
        f.write("\n\n")
        ...
        for param in param_info_list:
            ...
            if param in convert_dict:
                convert_param = convert_dict[param]
                if convert_param in explainColumnDict:
                    f.write("    /**\n")
                    f.write("     * {}\n".format(param))
                    f.write("     */\n")
                    f.write("    @ExcelProperty(value = \"{}\")\n".format(param))
                    f.write("    private {} {};".format("String", to_camelcase(explainColumnDict[convert_param])))
                    f.write("\n\n")
                    propertyList.append(to_camelcase(explainColumnDict[convert_param]))
                else:
                    for k, v in explainColumnDict.items():
                        if convert_param in k and to_camelcase(v) not in propertyList:
                            f.write("    /**\n")
                            f.write("     * {}\n".format(param))
                            f.write("     */\n")
                            f.write("    @ExcelProperty(value = \"{}\")\n".format(param))
